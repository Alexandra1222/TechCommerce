...
const data = {
  ...
  categories: () => {
    return {
      ...
      payload: CategoriesService.getAll({ populate: true }),
    };
  },
};
...
module.exports = {
  default: (req, res) => {
    ...
    const displayData = data[query.option]
      ? data[query.option]()
      : data['products']();
    const selection = displayData ? query.option : 'products';
  ...
  categories: {
    ...
    edit: (req, res) => {
      ...
      console.log('MARTIN_LOG=> category', category.payload);

      if (category.code === 'ERROR') {
        return res.render(path.resolve(__dirname, '../views/web/error'));
      }

      const categories = CategoriesService.getAll({ populate: true }).filter(a=>a.id !== category.payload.id);
      console.log("MARTIN_LOG=> categories", categories);
      res.render(
